...
layout = dbc.Col([
       ...
       dbc.Row([
           dbc.Col(dbc.Card(dcc.Graph(id='graph2'),style={'padding':'10px','margin-left':'10px'}),width=6),
           dbc.Col(dbc.Card(dcc.Graph(id='graph3'),style={'padding':'10px'}),width=3),
           dbc.Col(dbc.Card(dcc.Graph(id='graph4'),style={'padding':'10px','margin-right':"20px"}),width=3)
       ])
    ])
...
def pie_despesa(data_despesa,despesa):
    ...
    # fig.update_layout(margin=graph_margin, template=template_from_url(theme),height=350)
    fig.update_layout(margin=graph_margin, height=350)
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
